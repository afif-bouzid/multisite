...
import 'franchisor_kiosk_view.dart';
...
class _FranchisorDashboardViewState extends State<FranchisorDashboardView>
  ...
  void initState() {
    ...
    // Ajoute un listener pour redessiner l'interface quand l'onglet change
    _tabController.addListener(() {
      if (_tabController.indexIsChanging) {
        setState(() {}); // Met à jour l'état pour changer le FAB
      }
    });
  }
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: const [
            ...
            Tab(icon: Icon(Icons.lan), text: "Structure Borne"),
  ...
  // Gère l'affichage du bouton flottant en fonction de l'onglet actif
  Widget? _buildFloatingActionButton(BuildContext context, int index) {
    switch (index) {
      ...
      case 1: // Structure Borne
        return FloatingActionButton.extended(
          icon: const Icon(Icons.add),
          label: const Text("Nouvelle Catégorie"),
          onPressed: () => KioskView.showCategoryDialog(
              context), // Appelle la méthode statique
        );
      case 2: // Produits
        return FloatingActionButton.extended(
          icon: const Icon(Icons.add),
      ...
      case 5: // Filtres
        return FloatingActionButton.extended(
          ...
          onPressed: () => FiltersView.showFilterDialog(
              context), // Appelle la méthode statique
        );
      default:
        return null;
