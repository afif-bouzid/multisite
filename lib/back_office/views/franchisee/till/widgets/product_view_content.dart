...
class BouncingButton extends StatefulWidget {
  ...
  const BouncingButton(
      {super.key, required this.child, this.onTap, this.haptic = false});
  @override
...
class _BouncingButtonState extends State<BouncingButton>
  ...
  void initState() {
    ...
    _controller = AnimationController(
        vsync: this, duration: _duration, lowerBound: 0.96, upperBound: 1.0);
...
class _ProductViewContentState extends State<ProductViewContent> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _precacheAllActiveImages());
  }

  void _autoSelectFirstFilter(KioskCategory cat) {
    final validFilters = _getValidFiltersForCategory(cat);
    _selectedKioskFilterId =
        validFilters.isNotEmpty ? validFilters.first.id : null;
  }
  ...
  List<KioskFilter> _getValidFiltersForCategory(KioskCategory cat) {
    ...
    final activeFilterIds =
        activeProducts.expand((p) => p.kioskFilterIds).toSet();
    return cat.filters.where((f) => activeFilterIds.contains(f.id)).toList();
  }

  List<KioskCategory> _getValidCategories() {
    final visibleProducts = widget.posData.products.where((p) =>
        !p.isIngredient &&
        (widget.posData.menuSettings[p.productId]?.isVisible ?? false));
    final visibleFilterIds =
        visibleProducts.expand((p) => p.kioskFilterIds).toSet();
  ...
  List<MasterProduct> _getFilteredProducts() {
    return widget.posData.products.where((p) {
      ...
      if (_selectedKioskCategoryId != null) {
        ...
        if (!p.kioskFilterIds.any((id) => catFilterIds.contains(id))) {
          return false;
        }
    ...
    if (widget.isTablet) {
      return Row(
        ...
        children: [
          ...
          Expanded(
              child: Container(color: const Color(0xFFF5F5F5), child: content)),
        ],
      );
    }
    return content;
  ...
  Widget _buildFastSidebar() {
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: categories
            .map((cat) => _buildSidebarTile(
                    cat.name.toUpperCase(), _selectedKioskCategoryId == cat.id,
                    () {
                  setState(() {
  ...
  Widget _buildOptimizedGrid(List<MasterProduct> products) {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildUltraFastCard(product, settings,
            onTap: settings.isAvailable
                ? () => _handleProductTap(product, settings)
                : null);
      },
    );
  }
                ...
                children: [
                  if (product.photoUrl != null && product.photoUrl!.isNotEmpty)
                    ...
                    Container(
                        color: placeholderColor,
                        child: Icon(
                            isContainer ? Icons.folder : Icons.restaurant,
                            color: Colors.black12,
                            size: 50)),
                  if (isContainer)
                    Positioned(
...
class _ProductOptionsPageState extends State<ProductOptionsPage> {
  ...
  void initState() {
    ...
    if (widget.product.ingredientProductIds.isNotEmpty) {
      final local = widget.allProductsRef
          .where(
              (p) => widget.product.ingredientProductIds.contains(p.productId))
          .toList();
  ...
  Future<void> _fetchIngredients() async {
    try {
      ...
      final fetched = snapshot.docs
          .map((d) => MasterProduct.fromFirestore(d.data(), d.id))
          .toList();
  ...
  void _updateQuantity(ProductSection section, SectionItem item, int delta) {
    ...
    setState(() {
      ...
      final bool isRadio =
          typeLower.contains('unique') || typeLower.contains('radio');
      final bool isIncremental = typeLower.contains('increment') ||
          typeLower.contains('quantity') ||
          typeLower.contains('compteur');
      if (isRadio) {
        if (delta > 0) {
          sectionMap.clear();
        ...
        if (newQty == 0) {
          sectionMap.remove(item.product.id);
        } else {
          sectionMap[item.product.id] = newQty;
        }
      }
    ...
    for (var section in _sortedSections) {
      ...
      if (totalQty < section.selectionMin) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text("⚠️ OBLIGATOIRE : ${section.title.toUpperCase()}",
                style:
      ...
      itemsMap.forEach((itemId, qty) {
        ...
        if (item != null) {
          final adjustedItem = SectionItem(
              product: item.product, supplementPrice: _getActualPrice(item));
          for (int i = 0; i < qty; i++) {
  ...
  Widget build(BuildContext context) {
    final bool isModif = widget.initialOptions != null ||
        widget.initialRemovedIngredientIds != null;
            ...
            child: Stack(
              ...
              children: [
                ...
                if (_baseIngredients.isNotEmpty)
                  Align(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      label: Text(
                        _removedIngredientIds.isEmpty
                            ? "MODIFIER INGRÉDIENTS"
                            : "MODIFIÉ (${_removedIngredientIds.length})",
                        style: const TextStyle(
                ...
                Positioned(
                  ...
                  child: IconButton(
                    icon:
                        const Icon(Icons.close, color: Colors.white, size: 44),
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                if (_removedIngredientIds.isNotEmpty &&
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.circular(20),
                        border:
                            Border.all(color: Colors.red.shade100, width: 2.5)),
                    child: Row(
                      children: [
                ...
                ..._sortedSections
                    .map((section) => _buildSectionBlock(section)),
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                    backgroundColor: isFormValid
                        ? (isModif ? AppColors.bkBlue : AppColors.bkGreen)
                  ...
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text(isModif ? 'ENREGISTRER' : 'AJOUTER AU PANIER',
                          style: TextStyle(
                              fontSize: 28,
                              fontWeight: FontWeight.w900,
                              color: isFormValid
                      ...
                      Container(
                          width: 3,
                          height: 40,
                          color: isFormValid ? Colors.white38 : Colors.grey),
                      const SizedBox(width: 40),
        ...
        GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final int totalInSection =
                sectionMap.values.fold(0, (sum, q) => sum + q);
            final bool canAdd = section.selectionMax <= 0 ||
                totalInSection < section.selectionMax;
            if (isIncremental) {
              ...
              return AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: isSelected
                      ? (isRadio ? Colors.grey.shade100 : Colors.blue.shade50)
                      : Colors.white,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                      color: isSelected
                          ? (isRadio ? Colors.black : AppColors.bkBlue)
...
class _IngredientCustomizationDialogState
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      title: const Text("INGRÉDIENTS INCLUS",
          style: TextStyle(
              fontWeight: FontWeight.w900, fontSize: 26, letterSpacing: 0.8)),
      content: SizedBox(
        width: 600,
        height: 500,
        child: ListView.separated(
