...
class CompositeProductDialog extends StatefulWidget {
  ...
  // --- PARAMÈTRES POUR LA MODIFICATION ---
...
class _CompositeProductDialogState extends State<CompositeProductDialog> {
  ...
  void initState() {
    ...
    // 1. Initialisation de l'option (Modification ou Création)
    ...
    // 2. Initialisation des choix existants (Modification)
    ...
    // 3. Initialisation des ingrédients retirés (Modification)
    if (widget.initialRemovedIngredients != null) {
      _removedIngredientProductIds.addAll(widget.initialRemovedIngredients!);
    }

    // 4. On charge les sections
    _loadSections();

    // 5. Chargement des données annexes
  ...
  // --- LOGIQUE DE CHARGEMENT ET TRI DES SECTIONS ---
  void _loadSections() {
    setState(() => _isLoadingSections = true);

    // 1. On détermine QUELLES sections on doit afficher
    //    (Celles définies par l'option choisie, ou par défaut le produit)
    ...
    // 2. On récupère les objets sections correspondants
    List<ProductSection> sections = widget.allSections
        .where((s) => sectionsToDisplayIds.contains(s.sectionId))
        .toList();

    // 3. LE TRI STRICT (Identique à product_view_content.dart)
    //    On utilise l'ordre défini dans le MasterProduct pour la cohérence visuelle.
    sections.sort((a, b) {
      int indexA = widget.product.sectionIds.indexOf(a.sectionId);
      int indexB = widget.product.sectionIds.indexOf(b.sectionId);

      // Si l'ID n'est pas trouvé dans le Master (ex: section spécifique à une option),
      // on le met à la fin (999).
            ...
            onSelected: (selected) {
              if (selected) {
                setState(() {
                  ...
                  _selectedOptions
                      .clear(); // Reset des choix si changement de formule
                });
                _loadSections(); // Recharge avec le bon ordre Master
              }
            },
          );
        },
  ...
  void _validateAndClose() {
    ...
    final cartItem = CartItem(
      ...
      removedIngredientNames: _baseIngredients
          ...
          .map((p) => p.name)
          .toList(),
      // Assurez-vous d'avoir ajouté optionId à votre modèle CartItem
